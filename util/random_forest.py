...
def random_forest(filepath, n_estimators: int = 100, apply_smote=False, smote_ratio=0.3, k_neighbors=5, apply_undersampling= False, under_ratio=0.5):
    """
    Random forest classifier. Classic random forest used to predict melanoma cases based on traning or test data.
    """
    ...
    for fold, (tr, te) in enumerate(kf.split(X, y, groups=groups), start=1):
        ...
        # Create a Random forest classifier, train and fit one per fold 
